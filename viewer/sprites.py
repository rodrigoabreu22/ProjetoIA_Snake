...
class GameStateSprite(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        for x, y_dict in self.player.sight.items():
            for y in y_dict.keys():
                # Create a temporary surface with an alpha channel
                temp_surface = pygame.Surface((self.SCALE, self.SCALE), pygame.SRCALPHA)
                # Fill the temporary surface with red color and set the alpha value (0-255)
                temp_surface.fill((255, 0, 0, 60))  # 60 is the alpha value for reduced opacity
                # Blit the temporary surface onto the main surface
                self.image.blit(temp_surface, (int(x) * self.SCALE, int(y) * self.SCALE))
